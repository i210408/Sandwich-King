...
public class FoodPrepTableController : MonoBehaviour
{
    ...
    IEnumerator Serve()
    {
        ...
        if (explosionPrefab != null && plate != null)
        {
            ...
            Vector3 explosionPosition = new Vector3(platePosition.x, platePosition.y, platePosition.z - 1); // Ensure the explosion is on top of other elements

            // Instantiate the explosion prefab and make it play only once
            GameObject explosion = Instantiate(explosionPrefab, explosionPosition, Quaternion.identity);

            // Ensure the explosion is rendered on top
            SpriteRenderer explosionRenderer = explosion.GetComponent<SpriteRenderer>();
            if (explosionRenderer != null)
            {
                explosionRenderer.sortingLayerName = "Foreground"; // Ensure this layer is on top
                explosionRenderer.sortingOrder = 10; // Ensure this order is higher than other objects
            }
